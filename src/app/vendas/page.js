...
import { ShoppingBag, Plus, Calendar, CheckCircle2, AlertCircle, Clock, DollarSign, Package, X } from 'lucide-react';
import { formatCurrency, formatDate } from '@/lib/formatters';

export default function VendasConsignacoesPage() {
  ...
  const handleCreateItemChange = (idx, field, val) => {
    ...
    if (field === 'productId') {
      ...
      if (prod) {
        // Estimar preço de venda sugerido pela margem
        copy[idx].unitPrice = '25.00';
      }
    }
    setCreateItems(copy);
  };
  ...
  const addCreateItemRow = () => {
    setCreateItems([...createItems, { productId: products[0]?.id || '', quantitySent: '1', unitPrice: '20.00' }]);
  };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <div>
          <h1 className="page-title">Remessas & Acertos</h1>
          <p className="page-subtitle">Acompanhe pacotes enviados, devoluções e feche acertos com os pontos de venda</p>
        </div>
        <button className="btn btn-primary" onClick={openCreate} disabled={salesPoints.length === 0 || products.length === 0}>
          <Plus size={16} /> Nova Consignação / Remessa
        </button>
      </div>
      ...
      {(salesPoints.length === 0 || products.length === 0) && (
        <div className="alert alert-warning" style={{ marginBottom: 24 }}>
          <AlertCircle size={18} />
          <div>Para criar remessas, você precisa ter pelo menos <strong>1 Ponto de Venda</strong> e <strong>1 Produto</strong> cadastrado no sistema.</div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <ShoppingBag />
          <div className="empty-state-title">Nenhuma remessa enviada</div>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: 8 }}>
            Monte seu primeiro pacote de peças para deixar em uma banca ou feira.
          </p>
        </div>
      ) : (
            ...
            return (
              <div key={c.id} className="card" style={{ padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div className="actions-row">
                      ...
                      <button className="btn btn-ghost btn-sm btn-icon" onClick={() => deleteConsignment(c.id)}><X size={14} /></button>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ marginTop: 16, borderTop: '1px solid var(--border-primary)', paddingTop: 16 }}>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>Itens deste Pacote:</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 10 }}>
                    {c.items.map((item) => (
                      <div key={item.id} style={{ background: 'var(--bg-secondary)', padding: 10, borderRadius: 8, fontSize: '0.85rem', border: '1px solid var(--border-primary)' }}>
                        <div style={{ fontWeight: 600 }}>{item.product?.name || 'Peça'}</div>
                        <div style={{ color: 'var(--text-secondary)', marginTop: 4 }}>Enviados: <strong>{item.quantitySent}</strong> unid. × {formatCurrency(item.unitPrice)}</div>
                        {(isSettled || isPartial || item.quantitySold > 0 || item.quantityReturned > 0) && (
                          <div style={{ display: 'flex', gap: 10, marginTop: 6, fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            ...
            <form onSubmit={handleSettleSubmit}>
              <div className="modal-body">
                <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 16 }}>
                  Informe a contagem real na banca: o que foi vendido entra no caixa, o que foi devolvido volta ao seu estoque pronto e perdas são deduzidas.
                </p>
