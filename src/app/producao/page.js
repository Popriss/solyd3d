...
export default function ProducaoPage() {
  ...
  const availableRolls = rolls.filter(r => r.status === 'AVAILABLE' || r.status === 'LOW');
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Ordens de impressão e acompanhamento</p>
        </div>
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="10" style={{ textAlign: 'center', padding: 40 }}><div className="spinner" style={{ margin: '0 auto' }}></div></td></tr>
            ) : orders.length === 0 ? (
              <tr><td colSpan="10">
                <div className="empty-state">
                  <Factory />
                  <div className="empty-state-title">Nenhuma ordem de produção</div>
                  <div className="empty-state-text">Cadastre produtos e filamentos primeiro, depois crie ordens</div>
                </div>
              </td></tr>
            ) : orders.map(order => {
              const StatusIcon = statusIcons[order.status];
              return (
                <tr key={order.id} style={order.isFailure ? { background: 'rgba(251,113,133,0.04)' } : {}}>
                  <td><strong>#{order.id}</strong></td>
                  <td>{order.product?.name || '-'}</td>
                  <td>{order.filamentRoll ? `${order.filamentRoll.material} ${order.filamentRoll.color}` : '-'}</td>
                  <td>{order.machine?.name || '-'}</td>
                  <td>{order.actualWeightG ? formatWeight(order.actualWeightG) : '-'}</td>
                  <td>{order.actualPrintMinutes ? formatDuration(order.actualPrintMinutes) : '-'}</td>
                  <td>{order.totalCost ? formatCurrency(order.totalCost) : '-'}</td>
                  <td>
                    <span className={`badge ${statusBadgeMap[order.status]}`}>
                      <StatusIcon size={12} />
                      {orderStatusLabels[order.status]}
                    </span>
                    {order.isFailure && <span className="badge badge-rose" style={{ marginLeft: 4 }}>Prejuízo</span>}
                  </td>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  <label className="form-label">Rolo de Filamento</label>
                  <select className="form-select" value={form.filamentRollId} onChange={e => setForm({ ...form, filamentRollId: e.target.value })} required>
                    ...
                    {availableRolls.map(r => <option key={r.id} value={r.id}>{r.material} {r.color} — {r.brand} ({formatWeight(r.remainingWeightG)} restante)</option>)}
                  </select>
                </div>
                <div className="form-group">
            ...
            <form onSubmit={handleStatusUpdate}>
              <div className="modal-body">
                ...
                {statusForm.status === 'FAILED' && (
                  <div className="alert alert-danger"><AlertTriangle size={16} /><span>O material será deduzido do estoque e contabilizado como <strong>prejuízo financeiro</strong>.</span></div>
                )}
              </div>
              <div className="modal-footer">
